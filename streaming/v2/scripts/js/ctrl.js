...
function CheckStream()
{
    ...
    if(!APP.endsWith("dse.php")){
        ...
        else
        {
            ...
            ButtonWrapper.appendChild(ButtonDSK);
            ...
            ButtonWrapper.appendChild(ButtonAPP);

            var Spacer = document.createElement("div");
                Spacer.setAttribute("class", 'spacer');
            ButtonWrapper.appendChild(Spacer);

            var BODY = document.body.childNodes[0];
            document.body.insertBefore(Comment,BODY);
            document.body.insertBefore(ButtonWrapper,BODY);
        }
    }
}
...
function Save(what){
	...
	var save_form = document.createElement('form');
		...
		save_form.setAttribute('action','../scripts/php/save.php?str='+STR);
...
function Delete(what){
	...
	var delete_form = document.createElement('form');
		...
		delete_form.setAttribute('action','../scripts/php/delete.php?str='+STR);
...
//Show Runner ---------->
function Run(STATE){
    let CURRENT = document.getElementById('run-state');
	const DATA = {type:"sr", cmd:STATE};
    //Fade-in Mic MIC Alert
    if (TOP_APP.endsWith("pilotage.php") && CURRENT.value == 'welcome' && STATE != "welcome"){ alert('\u26A0 Get ready to fade-in Mic MIX \u26A0'); }
    CURRENT.value = STATE;
	Update(DATA);
}
...
//Open Broadcast Channels for Messages
var lmc = new BroadcastChannel(STR+'-local-messages-channel');

//Load Message sent in PREVIOUS MESSAGE
...
function Insert(TYPE){
    if(TYPE == "QR"){ CLASS = "qrcode"; TXT = "du lien"; var SRC="https://api.qrserver.com/v1/create-qr-code/?size=540x540&charset-source=UTF-8&ecc=L&margin=20&format=png&data="; }
	...
	else
	{
        ...
        var IMG = '<img class="'+CLASS+'" src="'+SRC+'">';
