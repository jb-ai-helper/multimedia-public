...
function error_function($error_level,$error_message)
{
	...
	if($collection == "auto")
	{
		$onload = ' onLoad="Send(document.getElementById(\'SI1\'))"';
		...
		if(!is_dir("../data/".$collection)){ $collection = "ENPJJ"; $title_ref = '0000'; }
		//TITLE doesn't exist
		if(!is_file("../data/".$collection."/streams/".$title_ref.".txt")){ $title_ref = '0000'; }
...
<body class="panel"<?php  echo $onload; ?>>

<!-- Global Parameters -->
<div id="GlobalParameters">
	<div class="subsection">Paramètres Globaux<?php  if($Stream != "") echo " (".strtoupper($Stream).")"; ?></div><input class="hidden" id="stream-key" value="<?php  echo $Stream; ?>">
	<div id="test" class="parameter"><label class="switch" style="transform: scale(1);"><input onChange="Switch(this)" id="Marianne" type="checkbox" checked><span class="slider round"></span></label>Afficher la Marianne</div>
	<div class="linked">
