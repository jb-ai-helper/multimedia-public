...
<?php
	...
	//Get & Set Modes
	if (!empty($_GET["mode"]) && $_GET["mode"] == 'light'){ $Mode_head = '<link rel="stylesheet" href="/streaming/v2/scripts/css/light.css">'; $Mute = "muted"; }
	...
	elseif (!empty($_GET["mode"]) && $_GET["mode"] == 'transition'){ $Chapter = 'ON'; $Title_Chapter = "<h1>Titre du chapitre</h1>"; $PlayState = ""; $BackgroundState = "CUT"; }
	elseif (!empty($_GET["mode"]) && $_GET["mode"] == 'miniature'){ $IntroductionState = "CUT"; $BackgroundState = "CUT"; $PlayState = "autoplay"; }
	elseif (!empty($_GET["mode"]) && $_GET["mode"] == 'projector'){ $Mode_head = '<link rel="stylesheet" href="/streaming/v2/scripts/css/projector.css">'; $Mute = "muted"; }
	elseif (!empty($_GET["mode"]) && $_GET["mode"] == 'counter'){ $Mode_head = '<link rel="stylesheet" href="/streaming/v2/scripts/css/counter.css">'; $Mute = "muted"; }

    if (isset($_GET["counter"]))
    {
        $CounterStart = $_GET["counter"];
        if(!is_numeric($CounterStart)){ $CounterStart = 0; }
...
<body onLoad="<?php echo $OnLoad ?>">
	...
	<!-- Background (5) -->
	<video id="Background" class="fullscreen <?php echo $BackgroundState ?>" <?php echo $PlayState ?> muted loop><source src="../styles/videos/bkg-enpjj.webm" type="video/webm"></video>
	...
	<audio id="Transition" <?php echo $Mute ?></audio><source src="/streaming/v2/sounds/transition.mp3" type="audio/mpeg"></audio>
	...
	<!-- Video Credits (9) -->
	<video id="Credits" class="fullscreen OFF" onended="this.currentTime=this.duration" <?php echo $Mute ?>><source src="" type="video/webm"></video>
