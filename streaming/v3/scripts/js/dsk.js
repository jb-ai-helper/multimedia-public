...
var Logos_animation = 0;
var Background_animation = -1;
var StartScreen_animation = 0;
var TitleBanner_animation = 0;
var LowerThird_animation = 0;
var Chapter_animation = 0;
...
function initDSK(TARGET,MODE){
    ...
    for(let font in fonts){
        for(let face in fonts[font]){
            let asset = new FontFace(
                fonts[font][face].family, 
                'url(' + fonts[font][face].url + ')',
                {
                    weight: fonts[font][face].weight,
                    style: fonts[font][face].style,
                });
            document.fonts.add(asset);
            LoadingPromises.push(asset.load());
        }
    } 
    //Generate Layers
 ...
 function DrawDSK(TARGET,MODE){
    ...
    return function draw(currentTime){
        ...
        if (elapsed > interval){
            ...
            if(MODE == "stream"){
                Logos_animation = 1;
                StartScreen_animation = 0;
                TitleBanner_animation = 1;
                LowerThird_animation = 0;
                Chapter_animation = 0;
                videos['Background'].opacity = 0;
...
function DrawText(TEXT, CONTEXT){
    ...
    //Define X & Y + Account for external offsets
    var X; var Y;
    if(TEXT.offsetX === undefined){ X = TEXT.x; }
    else{ X = TEXT.x + TEXT.offsetX; }
    if(TEXT.offsetY === undefined){ Y = TEXT.y; }
    else{ Y = TEXT.y + TEXT.offsetY; }
    
    //Get default Variable
    var HorizontalPosition = X;
    var VerticalPosition = Y;
    ...
    TEXT.height = TotalTextHeight;

    //Calculate Total (Max) Text Width
    var LineWidth = 0;
    lines.forEach(function(line){
        if(line.match(/<\/?h[1-3]>/)){//Opening or closing tag
            var htag = line.match(/<\/?(h[1-3])>/)[1];
            line = line.replace(/<\/?h[1-3]>/g, '');
        ...
        //Mesure line
        LineWidth = Math.ceil(CONTEXT.measureText(line).width);
        
        console.log(line, LineWidth);
        
        TotalTextWidth = Math.max(TotalTextWidth, LineWidth);;
        //Reset if closing tag
        if(line.match(/<\/h[1-3]>/)){ CONTEXT.font = DefaultFont; }
    });
    TEXT.width = TotalTextWidth;
        
    //Setup Horizontal Position for the block of text
    if(TEXT.anchor.includes('center')){ HorizontalPosition = Math.ceil(X - (TEXT.width / 2)); }
    if(TEXT.anchor.includes('right')){ HorizontalPosition = Math.ceil(X - TEXT.width); }

    //Draw Frame Effect
    if(TEXT.frame !== undefined){
        let FrameFX = DeepCopy(TEXT.frame);
        //Draw Before
        if(FrameFX['before'] !== undefined){
            let x0 = Math.ceil(HorizontalPosition - FrameFX['before'].width);        
            let y0 = Y;
            
            //Account for Text Anchor in Vertical Position
            if(TEXT.anchor.includes('middle')){ y0 = Y - (TEXT.height/2); }
            if(TEXT.anchor.includes('bottom')){ y0 = Y - TEXT.height; }
            
            if(FrameFX['before'].width === undefined){ FrameFX['before'].width = TEXT.width; }
            if(FrameFX['before'].height === undefined){ FrameFX['before'].height = TEXT.height; }
            FrameFX['before'].origin = [ Math.round(x0), Math.round(y0) ];
            CONTEXT.fillStyle = GenerateFramFill(FrameFX['before'], CONTEXT);
            CONTEXT.fillRect(x0, y0, FrameFX['before'].width, FrameFX['before'].height);
        }
        //Draw Around
        if(FrameFX['around'] !== undefined){
            let x0 = HorizontalPosition;
            let y0 = Y;
            ...
            if(FrameFX['around'].width === undefined){ FrameFX['around'].width = TEXT.width; }
            if(FrameFX['around'].height === undefined){ FrameFX['around'].height = TEXT.height; }
            FrameFX['around'].origin = [ Math.round(x0), Math.round(y0) ];
            CONTEXT.fillStyle = GenerateFramFill(FrameFX['around'], CONTEXT);
            CONTEXT.fillRect(x0, y0, FrameFX['around'].width, FrameFX['around'].height);
        }
        //Draw After
        if(FrameFX['after'] !== undefined){
            let x0 = Math.floor(HorizontalPosition + TEXT.width);
            let y0 = Y;
            ...
            if(FrameFX['after'].width === undefined){ FrameFX['after'].width = TEXT.width; }
            if(FrameFX['after'].height === undefined){ FrameFX['after'].height = TEXT.height; }
            FrameFX['after'].origin = [ Math.round(x0), Math.round(y0) ];
            CONTEXT.fillStyle = GenerateFramFill(FrameFX['after'], CONTEXT);
            CONTEXT.fillRect(x0, y0, FrameFX['after'].width, FrameFX['after'].height);
        }
        //Reset default
    ...
    lines.forEach(function(line){
        ...
        //Adjusted Vertical Position (AVP) line by line
        var AVP = Math.ceil(VerticalPosition + PreviousLineHeight + CurrentOffset);//Default Anchor TOP
        if(TEXT.anchor.includes('middle')){ AVP = Math.ceil(VerticalPosition - (TEXT.height / 2) + PreviousLineHeight + CurrentOffset); }
        if(TEXT.anchor.includes('bottom')){ AVP = Math.ceil(VerticalPosition - TEXT.height + PreviousLineHeight + CurrentOffset); }
        PreviousLineHeight+= LineHeight;

        //Adjusted Horizontal Position (AHP) line by line
        var AHP = HorizontalPosition;
        var LineWidth = CONTEXT.measureText(line).width;
        if(TEXT.anchor.includes('center')){ AHP = Math.ceil(HorizontalPosition+(TEXT.width-LineWidth)/2); }
        if(TEXT.anchor.includes('right')){ AHP = Math.ceil(HorizontalPosition+(TEXT.width-LineWidth)); }

        
        CONTEXT.fillText(line, AHP, AVP);
...
function GenerateFramFill(FRAME, CONTEXT){
    var FramFill;
    if(typeof FRAME.fill === 'string'){ FramFill = FRAME.fill; }
    else if(typeof FRAME.fill === 'object'){
        if(FRAME.fill.type == 'linear'){
            //Calculate Coordinates
            let angle = (FRAME.fill.angle + 90) * (Math.PI / 180);
            let width = FRAME.width;
            let height = FRAME.height;
            
            let x0 = Math.round(width / 2 + (width / 2) * Math.cos(angle));
            let y0 = Math.round(height / 2 + (height / 2) * Math.sin(angle));
            let x1 = Math.round(width / 2 + (width / 2) * Math.cos(angle + Math.PI));
            let y1 = Math.round(height / 2 + (height / 2) * Math.sin(angle + Math.PI));
            
            //Offset by Orgin
            x0 = x0 + FRAME.origin[0];
            x1 = x1 + FRAME.origin[0];
            y0 = y0 + FRAME.origin[1];
            y1 = y1 + FRAME.origin[1];

            //Build gradient
            const linear_gradient = CONTEXT.createLinearGradient(x0, y0, x1, y1);
            let colors = FRAME.fill.color;
            Object.keys(colors).forEach(color => { linear_gradient.addColorStop(color, colors[color]); });
            FramFill = linear_gradient;
        }
    }
    return FramFill;
}
...
function DrawBackgound(PLUS){
    ...
    //Make sure the video can play (browser restructions)
...
function DrawLowerThird(){
    if(LowerThird_animation > 0){
        ctx['LowerThird'].clearRect(0, 0, screen.width, screen.height);
        //Translation
        var Translation = Object.assign({}, texts['LowerThird']['Translation']);
        DrawText(Translation, ctx['LowerThird']);
        //Function
        var Function = Object.assign({}, texts['LowerThird']['Function']);
            Function.offsetY = -Translation.height;
        DrawText(Function, ctx['LowerThird']);
        //Name
        var Name = Object.assign({}, texts['LowerThird']['Name']);
            Name.offsetY = -Translation.height - Function.height;
        DrawText(Name, ctx['LowerThird']);
        LowerThird_animation--;
    }
}
