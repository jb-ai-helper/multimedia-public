...
function SetUp(){
	...
	for (let Bubble of Bubbles){
		...
		Bubble.style.backgroundImage = "url('"+app+"/src/img/favicon.ico')";
...
{
	SetBubbles(ID);
	APPS = document.getElementsByClassName(ID);
	for(i=APPS.length-1;i>=0;i--) { (APPS[i].classList.contains("start") == true) ? APPS[i].classList.remove("start") : APPS[i].classList.add("start"); }
}
