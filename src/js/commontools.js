...
// Arrays ---------->
    ...
    var french_days = new Array();
		french_days[0]="dimanche",
		french_days[1]="lundi",
		french_days[2]="mardi",
		french_days[3]="mercredi",
		french_days[4]="jeudi",
		french_days[5]="vendredi",
		french_days[6]="samedi";
	
    var french_month = new Array();
		french_month[0]="janvier",
		french_month[1]="février",
		french_month[2]="mars",
		french_month[3]="avril",
		french_month[4]="mai",
		french_month[5]="juin",
		french_month[6]="juillet",
		french_month[7]="août",
		french_month[8]="septembre",
		french_month[9]="octobre",
		french_month[10]="novembre",
		french_month[11]="décembre";
...
function CheckBrowser(){
    ...
    else{ browser="No browser detection"; }
    
    if(browser == "Chrome" || browser == "Edge"){ return true; }
    else{ alert('Votre navigateur ('+browser+') n\'est pas compatible avec cette application.\r\nMerci d\'utiliser Chrome ou Edge...'); return false; }
}

//Normalize a name (no accents / just letters and numbers)
function NormalizeName(NAME){
    var NewName = NAME.normalize('NFD').replace(/[\u0300-\u036f]/g, '');//retire les accents
        NewName = NewName.replace(/[^a-z0-9_]/gi, '');//retire tout ce qui n'est pas des chiffres, des lettres ou des underscores
    return NewName;
}
...
function CorrectKeys(e){	
	...
	else if(e.which>=48 && e.which<=57 && e.shiftKey){ return true; } //Numbers + Shift
...
function FormateDate(DATE,FORMAT){
	...
	if(FORMAT == 1){
		...
		var FormatedDate = day + "&nbsp;" + date + " " + month;	
	}
	else{
        month = parseInt(month) +1;//Account for getMonth starting at 0
...
//Generate SubRefs
function GenerateSubRef(LENGTH){
	
	if(!LENGTH) LENGTH = 4;
	var SubRef = Math.floor(Math.random() * 10000).toString();
	while(SubRef.length<LENGTH){ SubRef = "0"+SubRef; }
	
	if(document.getElementById(SubRef)) GenerateSubRef(LENGTH);
	else return SubRef;
}
...
//Create function to get element ending with specific TEXT
function getElementByIdEndingWith(text) {
...
//Create function to "Deep" Copy an Object
function DeepCopy(obj) {
    //Check if OBJ or TABLE
...
//Create Function to convert OBJ to XML
...
function MoveUp(what){
	var ELEMENT = what.parentNode;
    var PARENT = ELEMENT.parentNode;
	var SEQUENCE = Array.prototype.slice.call(PARENT.children);
	...
	if(INDEX > 0)
	{
		PARENT.insertBefore(SEQUENCE[INDEX],SEQUENCE[INDEX - 1]);
		INDEX--;
	}
}
...
function MoveDown(what){
	...
	{
		PARENT.insertBefore(SEQUENCE[INDEX+1],SEQUENCE[INDEX]);
		INDEX++;
	}
}
...
function waitForContentToLoad(element, callback) {
  const isLoaded = () => {
    ...
    const allIframesLoaded = Array.from(iframes).every(iframe => iframe.contentWindow);
    
    // Retourner vrai si tout est chargé
    return allImagesLoaded && allIframesLoaded;
  };
  ...
  const wait = () => {
    if (isLoaded()) {
      ...
      // Réessayer au prochain tick si pas prêt
